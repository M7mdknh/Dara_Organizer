...
const GENERAL_TABS = ["Users", "AI & Confidence", "Export defaults"] as const;
const ADVANCED_TABS = ["Dialects", "Languages", "Categories", "Taxonomies"] as const;
type Tab = (typeof GENERAL_TABS)[number] | (typeof ADVANCED_TABS)[number];

export default function SettingsPage() {
  const [tab, setTab] = useState<Tab>("Users");
  const [showAdvanced, setShowAdvanced] = useState<boolean>((ADVANCED_TABS as readonly string[]).includes(tab));
  ...
  return (
    <div>
      <PageHeader title="Settings" subtitle="Account, AI configuration, and export defaults" />
      ...
      {(ADVANCED_TABS as readonly string[]).includes(tab) && (
        <div className="mb-5 flex gap-1 border-b border-border overflow-x-auto">
          {ADVANCED_TABS.map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-3 py-2 text-sm whitespace-nowrap border-b-2 -mb-px ${tab === t ? "border-accent text-accent font-medium" : "border-transparent text-muted hover:text-foreground"}`}>
              {t}
      ...
      <div className="mt-8 border-t border-border pt-4">
        <button onClick={() => setShowAdvanced((s) => !s)} className="text-xs text-muted hover:text-foreground underline mb-3">
        ...
        {showAdvanced && (
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              {ADVANCED_TABS.map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className="text-sm rounded-lg border border-border px-3 py-1.5 hover:bg-foreground/5"
                >
                  {t}
                </button>
              ))}
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <Link href="/sources" className="card p-4 hover:border-accent transition-colors block">
                <div className="font-medium text-sm">Media &amp; Sources</div>
                <div className="text-xs text-muted mt-1">Every import as a traceable source; upload files directly</div>
              </Link>
              <Link href="/collections" className="card p-4 hover:border-accent transition-colors block">
                <div className="font-medium text-sm">Collections</div>
                <div className="text-xs text-muted mt-1">Curated groupings of records for review or export</div>
              </Link>
            </div>
          </div>
        )}
